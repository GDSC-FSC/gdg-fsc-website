import { delegate } from '../../../packages/shared/decorators/src/lib/delegate/delegate.ts';
...
describe('delegate', () => {
  it('should make sure error thrown when decorator not set on method', () => {
    try {
      ...
      (Error.isError(e))
        ...
        : expect('@delegate is applicable only on a methods.').toBe(String(e))
  ...
  it('should delegate method with same key invocation - default key serialization - many args', async () => {
    ...
    const res = await Promise.all([t.foo(1, 1, 1, 1), t.foo(1, 1, 1, 2), t.foo(1, 1, 1, 1)]);
