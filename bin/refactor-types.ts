...
function updateExistingImport(existingImport: ImportDeclaration, movedNames: string[]): void {
...
async function main() {
  logger.info('Starting type refactor...');
  if (options.dryRun) {
  ...
  if (files.length === 0) {
    ...
    logger.info(`  Ignored: ${ignore.join(', ')}`);
    return;
  }

  const project = createProject(files);
  logger.info(`Scanning ${project.getSourceFiles().length} files...`);
...
try {
  ...
  logger.error('An error occurred during type refactor:', err);
  process.exit(1);
}
