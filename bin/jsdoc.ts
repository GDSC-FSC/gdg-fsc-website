...
class JSDocGenerator {
  ...
  constructor(apiKey: string, config: Partial<GenerationConfig> = {}) {
    ...
    this.concurrencyLimiter = new ConcurrencyLimiter(this.config.maxConcurrency ?? 3);
  }
  ...
  private async makeApiCall(fileContent: string, attempt = 1): Promise<string> {
    try {
      ...
      if (attempt < (this.config.retryAttempts ?? 2) + 1) {
        logger.warn(`🔄 Attempt ${attempt} failed, retrying in ${this.config.retryDelay}ms...`);
  ...
  async processFile(
    ...
    try {
      ...
      logger.error(`❌ Failed to process ${inputPath}: ${errorMessage}`);
  ...
  async processFiles(filePaths: string[], overwrite = false): Promise<ProcessingResult[]> {
    ...
    logger.info('\n📊 Processing Summary:');
    logger.success(`✅ Successful: ${successful.length}/${results.length} files`);
    logger.error(`❌ Failed: ${failed.length}/${results.length} files`);
    logger.info(`⏱️  Total time: ${totalDuration}ms`);
    logger.info(`⚡ Average time per file: ${Math.round(totalDuration / results.length)}ms`);
  ...
  async scanGitFiles(
    ...
    const supportedExtensions = new Set(['.ts', '.js', '.tsx', '.jsx']);
    ...
    try {
      ...
      let gitOutput = execSync('git ls-files', FILE_OPTS);

      if (includeUntracked) {
        const untrackedOutput = execSync('git ls-files -o --exclude-standard', FILE_OPTS);
        gitOutput += '\n' + untrackedOutput;
      }
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      logger.warn(
        `⚠️  Git not available or not in a Git repository (${errorMessage}), falling back to directory scanning`,
      );
  ...
  async processDirectory(
    ...
    if (useGit) {
      try {
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        logger.warn(`⚠️  Git scanning failed (${errorMessage}), falling back to directory scanning`);
    ...
    if (files.length === 0) {
      logger.info(`📁 No supported files found in ${directoryPath}`);
      return [];
    }
...
function extractExcludePatterns(args: string[]): string[] {
...
function parseArgs(args: string[]): CLIOptions {
  ...
  return {
    ...
    retryAttempts,
    targetDirectory,
    excludePatterns: extractExcludePatterns(args),
    files: extractFiles(args),
  };
}

function validateOptions(options: CLIOptions): void {
  if (Number.isNaN(options.maxConcurrency) || options.maxConcurrency < 1) {
    logger.error('❌ Invalid concurrency value. Must be a positive integer.');
    process.exit(1);
  }

  if (Number.isNaN(options.retryAttempts) || options.retryAttempts < 0) {
    logger.error('❌ Invalid retry attempts value. Must be a non-negative integer.');
    process.exit(1);
  }
}
...
async function processFiles(
  ...
  if (options.targetDirectory) {
    return generator.processDirectory(
  ...
  for (const file of options.files) {
    ...
    const ext = extname(file);
    if (!supportedExtensions.has(ext)) {
      logger.error(`❌ Unsupported file type: ${file} (${ext})`);
      continue;
    }
  ...
  return generator.processFiles(validFiles, options.overwrite);
}
...
async function main() {
  ...
  try {
    const results = await processFiles(generator, options);
